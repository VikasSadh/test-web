...
class AddIndividualUserModal extends Component {
 constructor(props) {
    super(props);
		this.state = { fields: {}, errors: {}, stateoption:[], cityoption:[],stateandcityoption:[],user_area:'',loadingp: false, suggestions: [],vaild_mobileno:'',vaild_emailid:'' }
...
componentDidMount() {
	...
	componentWillReceiveProps(newProps) {    
	  ...
	  if(newProps.isOpen===false) { 
		...
		fields['user_alt_mobileno'] = '';
		fields['user_area'] = '';
		fields['user_address'] = '';
		fields['user_landmark'] = '';
		fields['user_pincode'] = '';
		fields['user_longitude'] = '';
		fields['user_latitude'] = '';
		let errors = {};
 ...
 handleChange(e) {
		...
		if(fields["user_mobileno"]) {
			const postdata = { user_mobileno: e.target.value };
				axios.post(API_URL+'/api/admin/check_mobileno', postdata).then(function(response){
		...
		if(fields["user_emailid"]) {
			const postdata = { user_emailid: e.target.value };
				axios.post(API_URL+'/api/admin/check_emailid', postdata).then(function(response){
 ...
 handleSubmit(e) {
	 let $this = this;
      ...
      if (this.validateForm()) {
		...
		axios.post(API_URL+'/api/admin/insertuser', PostData).then(function(response){
				 if(response.data.Statuscode==200) {
						    toastr.success('User', response.data.Message);
							// history.push('/ez-admin/individual-users');
							// $this.setState({ isModalOpen: !$this.props.isModalOpen, fade: !this.props.fade  });
        ...
        <Row>
			 <Col>
                ...
                <Modal isOpen={this.props.isOpen} toggle={this.toggle} className={'modal-lg'}>
                  <ModalHeader toggle={this.toggle}>Add Individual User </ModalHeader>
                  <ModalBody>
								<Row>
          <Col xs="12" sm="12">
            <Card>
